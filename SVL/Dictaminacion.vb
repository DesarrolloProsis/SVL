...
#Region "Methods"
    ...
    Private Sub AjusteColumnas()
        ...
        dgvEventos.Columns(2).Width = 120
        ...
        dgvEventos.Columns(16).IsVisible = False
    ...
    Private Sub CargarTodos()
        ...
        If idHora = 1 Then
            Dim eventos = From pre In ctx.Transacciones
                          ...
                          Where ((pre.Fecha = FechaEvento.AddDays(-1) And pre.Hora > Hora1) Or (pre.Fecha = FechaEvento And pre.Hora < Hora2)) And pre.NoPlaza = NumCaseta And pre.NoCarrilCapufe = idCarril And (pre.idTurno = idHora Or pre.idTurno = 0)
        ...
        Else
            Dim eventos = From pre In ctx.Transacciones
                          ...
                          Join t In ctx.Turnos
                                   On pre.idTurno Equals t.idTurno
                          Where pre.Fecha = FechaEvento And pre.NoPlaza = NumCaseta And pre.NoCarrilCapufe = idCarril And (pre.idTurno = idHora Or pre.idTurno = 0)
                          Order By pre.Hora Descending
                          Select
                               Evento = pre.NoEvento,
                               Hora = pre.Hora,
    ...
    Private Sub CargarPredefinidos()
        ...
        If idHora = 1 Then
            Dim preDefinidos = From pre In ctx.Transacciones
                               ...
                               Join t In ctx.Turnos
                                   On pre.idTurno Equals t.idTurno
                               Where ((pre.Fecha = FechaEvento.AddDays(-1) And pre.Hora > Hora1) Or (pre.Fecha = FechaEvento And pre.Hora < Hora2)) And pre.NoPlaza = NumCaseta And pre.NoCarrilCapufe = idCarril And (pre.idTurno = idHora Or pre.idTurno = 0) And pre.idPago <> 1 And pre.idPago <> 12 And pre.idPago <> 14 'And (tp.idPago = IAVE Or tp.idPago = Residente Or tp.idPago = VSC Or tp.idPago = Eludido)
                               Order By pre.Hora Descending
                               Select
                                     Evento = pre.NoEvento,
                                     Hora = pre.Hora,
        ...
        Else
            Dim preDefinidos = From pre In ctx.Transacciones
                               ...
                               Join t In ctx.Turnos
                                   On pre.idTurno Equals t.idTurno
                               Where pre.Fecha = FechaEvento And pre.NoPlaza = NumCaseta And pre.NoCarrilCapufe = idCarril And (pre.idTurno = idHora Or pre.idTurno = 0) And pre.idPago <> 1 And pre.idPago <> 12 And pre.idPago <> 14 'And (tp.idPago = IAVE Or tp.idPago = Residente Or tp.idPago = VSC Or tp.idPago = Eludido)
                               Order By pre.Hora Descending
                               Select
                                     Evento = pre.NoEvento,
                                     Hora = pre.Hora,
    ...
    Public Sub CargarSeleccion()
        ...
        If idHora = 1 Then
            ...
            Dim seleccion = From i In ctx.Transacciones
                            ...
                            Where ((i.Fecha = FechaEvento.AddDays(-1) And i.Hora > Hora1) Or (i.Fecha = FechaEvento And i.Hora < Hora2)) And i.NoPlaza = NumCaseta And (i.idTurno = idHora Or i.idTurno = 0) And i.NoCarrilCapufe = idCarril And (i.idPago = iav Or i.idPago = vs Or i.idPago = resi Or i.idPago = elu Or i.idPago = nor Or i.idPago = tarjC Or i.idPago = tarjD)
                            Order By i.NoEvento
                            Select
                                   Evento = i.NoEvento,
                                   Hora = i.Hora,
        ...
        Else
            ...
            Dim seleccion = From i In ctx.Transacciones
                            ...
                            Join t In ctx.Turnos
                                       On i.idTurno Equals t.idTurno
                            Where i.Fecha = FechaEvento And i.NoPlaza = NumCaseta And (i.idTurno = idHora Or i.idTurno = 0) And i.NoCarrilCapufe = idCarril And (i.idPago = iav Or i.idPago = vs Or i.idPago = resi Or i.idPago = elu Or i.idPago = nor Or i.idPago = tarjC Or i.idPago = tarjD)
                            Order By i.NoEvento
                            Select
                                   Evento = i.NoEvento,
                                   Hora = i.Hora,
            ...
            Dim x As Integer = dgvEventos.Rows.Count()
            dgvEventos.DataSource = seleccion.ToList()
            AjusteColumnas()
    ...
    Private Sub Buscar()
        ...
        Dim busqueda = From b In ctx.Transacciones
                       ...
                       Where b.Fecha = FechaEvento And b.NoPlaza = NumCaseta And (b.idTurno = idHora Or b.idTurno = 0) And b.NoCarrilCapufe = idCarril And b.NoEvento = Integer.Parse(txtBuscar.Text)
                       Order By b.NoEvento
                       Select
                             Evento = b.NoEvento,
                             Hora = b.Hora,
            ...
            If My.Computer.Network.Ping(IPVideo) Then
                ...
                Try
                    ...
                    With AxRMSInfo2
                        ...
                        .LoadRecordings(4, 4, "1#asdc") '4, 4, "1#asdc"
