        ...
        Try
            ...
            If Turno = 1 Then
                existen = (From e In db.Transacciones
                           Where ((e.Fecha = Fecha.AddDays(-1) And e.Hora > H1) Or (e.Fecha = Fecha And e.Hora < H2)) And e.NoCarrilCapufe = idCarril And (e.idTurno = Turno Or e.idTurno = 0) And e.NoPlaza = Caseta
                           Select e.NoEvento).Count()
            Else
                existen = (From e In db.Transacciones
                           Where e.Fecha = Fecha And e.NoCarrilCapufe = idCarril And (e.idTurno = Turno Or e.idTurno = 0) And e.NoPlaza = Caseta
                           Select e.NoEvento).Count()
            End If
